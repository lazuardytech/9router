...
async function setupTestContext(nodeData) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pod-compatible-provider-"));
...
function makeRequest(provider, overrides = {}) {
  return new Request("https://pod.local/api/providers", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      provider,
...
describe("compatible provider connections API", () => {
  ...
  it("allows multiple connections on the same compatible node", async () => {
    const ctx = await setupTestContext({
      ...
      prefix: "multi",
      apiType: "chat",
      baseUrl: "https://multi-connection.test/v1",
    });
    cleanup = ctx.cleanup;
